...
app.get("/buscar", async (req, res) => {
  ...
  try {
    ...
    // Llamada al API de MP con reintentos automáticos.
    // La API de MP es notoriamente intermitente: a veces responde con HTTP 200
    // pero `Listado` vacío sin error explícito (rate limit silencioso, sesión
    // expirada, etc). Sin reintento, el agente reporta "0 resultados" cuando
    // en realidad hay miles de licitaciones disponibles.
    //
    // Estrategia:
    //   - Hasta 3 intentos
    //   - Backoff incremental: 2s → 4s → 8s
    //   - Acepta "vacío legítimo" solo en intento 3 (después de retries)
    //   - Loguea cada intento para diagnóstico
    const fetchAll = async (extraParams, etiqueta = "") => {
      ...
      for (let intento = 1; intento <= MAX_INTENTOS; intento++) {
        try {
          ...
          if (!mpRes.ok) {
            console.warn(`[buscar:${etiqueta}] HTTP ${mpRes.status} intento ${intento}/${MAX_INTENTOS}`);
          } else {
            const data = await mpRes.json();
            const listado = Array.isArray(data.Listado) ? data.Listado : [];
            const cantidad = typeof data.Cantidad === "number" ? data.Cantidad : listado.length;
            ultimoTamanio = listado.length;
            console.log(`[buscar:${etiqueta}] intento ${intento} → ${listado.length} licitaciones (Cantidad=${cantidad})`);
            // Si trae datos, devolver. Si está vacío pero la API indica Cantidad>0 también es fail.
            if (listado.length > 0) return listado;
            // Vacío "real" en intento 1-2 → reintentar. Vacío en intento 3 → aceptar.
            if (intento === MAX_INTENTOS) {
              console.warn(`[buscar:${etiqueta}] ⚠️ Después de ${MAX_INTENTOS} intentos sigue vacío. Aceptando 0 resultados.`);
              return [];
            }
          }
        } catch (e) {
          if (e.name === "AbortError") throw e;
          console.warn(`[buscar:${etiqueta}] Error intento ${intento}/${MAX_INTENTOS}: ${e.message}`);
        }
        // Backoff antes del siguiente intento (sólo si no fue el último)
        if (intento < MAX_INTENTOS) {
          const espera = 2000 * intento; // 2s → 4s → 8s
    ...
    // Las 2 llamadas son secuenciales (NO paralelas) para evitar que la API
    // detecte "peticiones simultáneas" y rechace una. El extra delay es mínimo.
    const sinTipo = await fetchAll("", "activas");
    await new Promise(r => setTimeout(r, 500)); // mini pausa entre llamadas
    const conSC   = await fetchAll("&tipo=SC", "tipoSC");
    clearTimeout(timeoutId);

    console.log(`[buscar] TOTAL: sinTipo=${sinTipo.length} conSC=${conSC.length}`);
...
app.post("/buscar-general", async (req, res) => {
  ...
  try {
    ...
    // fetchAll con reintentos automáticos (igual lógica que /buscar para
    // tolerar intermitencia de la API de MP — ver /buscar para detalles).
    const fetchAll = async (extraParams, etiqueta = "") => {
      const usaEstado = !extraParams.includes("tipo=SC");
      const mpUrl = `https://api.mercadopublico.cl/servicios/v1/publico/licitaciones.json` +
                    `?${usaEstado ? "estado=activas&" : ""}ticket=${TICKET}${extraParams}`;
      ...
      for (let intento = 1; intento <= MAX_INTENTOS; intento++) {
        try {
          ...
          if (mpRes.ok) {
            const data = await mpRes.json();
            const listado = Array.isArray(data.Listado) ? data.Listado : [];
            console.log(`[buscar-org:${etiqueta}] intento ${intento} → ${listado.length} licitaciones`);
            if (listado.length > 0) return listado;
            if (intento === MAX_INTENTOS) return [];
          } else {
            console.warn(`[buscar-org:${etiqueta}] HTTP ${mpRes.status} intento ${intento}`);
          }
        } catch(e) {
          if(e.name==="AbortError") throw e;
          console.warn(`[buscar-org:${etiqueta}] Error intento ${intento}: ${e.message}`);
        }
        if (intento < MAX_INTENTOS) await new Promise(r => setTimeout(r, 2000 * intento));
      }
      return [];
    };

    // Secuencial para evitar rate limit por peticiones simultáneas
    const sinTipo = await fetchAll("", "activas");
    await new Promise(r => setTimeout(r, 500));
    const conSC   = await fetchAll("&tipo=SC", "tipoSC");
    clearTimeout(timeoutId);

    console.log(`[buscar-org] TOTAL: sinTipo=${sinTipo.length} conSC=${conSC.length}`);
...
app.get("/buscar-organismo", async (req, res) => {
  ...
  try {
    ...
    // Reintentar hasta 3 veces si la API devuelve vacío (intermitencia conocida)
    let licitaciones = [];
    const MAX_INTENTOS = 3;
    for (let intento = 1; intento <= MAX_INTENTOS; intento++) {
      try {
        ...
        if (mpRes.ok) {
          const data = await mpRes.json();
          const listado = Array.isArray(data.Listado) ? data.Listado : [];
          console.log(`[buscar-org-cod] intento ${intento} → ${listado.length} licitaciones`);
          if (listado.length > 0) { licitaciones = listado; break; }
        }
      } catch(e) {
        if (e.name === "AbortError") throw e;
        console.warn(`[buscar-org-cod] Error intento ${intento}: ${e.message}`);
      }
      if (intento < MAX_INTENTOS) await new Promise(r => setTimeout(r, 2000 * intento));
    }
    clearTimeout(timeoutId);
